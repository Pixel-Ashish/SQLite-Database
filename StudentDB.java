...
public class StudentDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String cmd="create table "+TABLE_NAME+" (name text primary key, location text, profession text)"; db.execSQL(cmd);
    }
    ...
    public String[] fetchdb(String nm){
        ...
        String query="select * from "+TABLE_NAME+" where name='"+nm+"'"; @SuppressLint("Recycle") Cursor cursor=db.rawQuery(query,null); if(cursor!=null){

            cursor.moveToFirst();

            ret[0]=cursor.getString(cursor.getColumnIndex("location")); ret[1]=cursor.getString(cursor.getColumnIndex("profession"));
        }
